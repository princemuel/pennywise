...
_this project uses [sqlx](https://crates.io/crates/sqlx) without an additional ORM and is set up for use with PostgreSQL._
...
Instead of using an ORM, that would introduce additional complexity, this project uses individual functions for reading and writing data from and to the database, e.g.:
...
Application tests will typically require test data to populate the database with, e.g. a set of entities to assert that the endpoint that returning all entities of that type works correctly. this project uses [fake](https://crates.io/crates/fake) for so that rules for creating fake data can be declared directly in the changesets:
