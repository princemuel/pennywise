...
/// A changeset representing the data that is intended to be used to either
/// create a new task or update an existing task.
///
/// Changesets are validatated in the [`create`] and [`update`] functions which
/// return an [Result::Err] if validation fails.
///
/// Changesets can also be used to generate fake data for tests when the
...
///
/// If no record can be found for the ID, a [`crate::Error::NoRecordFound`] will
/// be returned.
pub async fn load(
    id: Uuid,
    executor: impl sqlx::Executor<'_, Database = Postgres>,
...
///
/// If the data in the changeset isn't valid, a
/// [`crate::Error::ValidationError`] will be returned, otherwise the created
/// task is returned.
pub async fn create(
    task: TaskChangeset,
    executor: impl sqlx::Executor<'_, Database = Postgres>,
...
/// Updates a task in the database with the data in the passed
/// [`TaskChangeset`].
///
/// If the data in the changeset isn't valid, a
/// [`crate::Error::ValidationError`] will be returned, otherwise the updated
/// [`Task`] is returned. If no record can be found for the ID, a
/// [`crate::Error::NoRecordFound`] will be returned.
pub async fn update(
    id: Uuid,
    task: TaskChangeset,
    executor: impl sqlx::Executor<'_, Database = Postgres>,
