use std::sync::Arc;

use api_config::DatabaseConfig;
use rand::distr::Alphanumeric;
use rand::{Rng, rng};
// use regex::{Captures, Regex};
use sqlx::postgres::PgConnection;
use sqlx::{Connection, Executor};

use crate::{DbPool, connect_pool};

/// Sets up a dedicated database to be used in a test case.
///
/// This sets up a dedicated database as a fork of the main test database as
/// configured in `.env.test`. The database can be used in a test case to ensure
/// the test case is isolated from other test cases. The function returns a
/// connection pool connected to the created database. This function is
/// automatically called by the [`api-macros::db_test`] macro. The return
/// connection pool is passed to the test case via the
/// [`api-macros::DbTestContext`].
#[allow(unused)]
pub async fn setup_db(config: &DatabaseConfig) -> DbPool {
    let test_db_config = prepare_db(config).await;
    connect_pool(&test_db_config)
        .await
        .expect("Could not connect to database!")
}

/// Drops a dedicated database for a test case.
///
/// This function is automatically called by the [`api-macros::db_test`] macro.
/// It ensures test-specific database are cleaned up after each test run so we
/// don't end up with large numbers of unused databases.
pub async fn teardown_db(db_pool: DbPool) {
    let mut connect_options = db_pool.connect_options();
    let db_config = Arc::make_mut(&mut connect_options);

    drop(db_pool);

    let root_db_config = db_config.clone().database("postgres");
    let mut connection: PgConnection = Connection::connect_with(&root_db_config).await.unwrap();

    let test_db_name = db_config.get_database().unwrap();

    let query = format!("DROP DATABASE IF EXISTS {test_db_name}");
    connection.execute(query.as_str()).await.unwrap();
}

async fn prepare_db(config: &DatabaseConfig) -> DatabaseConfig {
    let db_config = config.connect_opts_with_db();
    let db_name = db_config
        .get_database()
        .expect("Database name must be specified in DATABASE_URL!");

    let root_db_config = db_config.clone().database("postgres");
    let mut connection: PgConnection = Connection::connect_with(&root_db_config)
        .await
        .expect("Failed to connect to root database!");

    let test_db_name = build_test_db_name(db_name);

    let query = format!("CREATE DATABASE {test_db_name} TEMPLATE {db_name}");
    connection.execute(query.as_str()).await.unwrap();

    let mut test_db_config = config.clone();
    test_db_config.name = test_db_name;

    // let regex = Regex::new(r"(.+)\/(.+$)").unwrap();
    // let test_db_url = regex.replace(&config.url(), |caps: &Captures| {
    //     format!("{}/{}", &caps[1], test_db_name)
    // });

    test_db_config
}

fn build_test_db_name(base_name: &str) -> String {
    let test_db_suffix: String = rng()
        .sample_iter(&Alphanumeric)
        .take(30)
        .map(char::from)
        .collect();
    format!("{base_name}_{test_db_suffix}").to_lowercase()
}

// fn parse_db_config(url: &str) -> PgConnectOptions {
//     PgConnectOptions::from_str(url).expect("Invalid DATABASE_URL!")
// }
