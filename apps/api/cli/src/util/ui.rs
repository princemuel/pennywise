...
impl<'a> UI<'a> {
    ...
    ///
    /// If `color` is `true`, output will be colored and formatted with emojis
    /// depending on the type of output. If `debug` is `true`, additional output
    /// will be printed in some cases, e.g. stack traces for errors.
    ///
    /// Example:
    /// ```
    ...
    fn indentation(&mut self) -> String { "  ".repeat(self.indentation) }
    ...
    ///
    /// If color output is enabled (see [`UI::new`]), the message will be
    /// formatted.
    ...
    ///
    /// If color output is enabled (see [`UI::new`]), the message will be
    /// formatted. If debug output is enabled (see [`UI::new`]), the error's
    /// stack trace will be printed as well.
    ...
    /// // >    Ran 5 migrations
    /// ```
    pub fn indent(&mut self) { self.indentation += 1; }
...
mod tests {
    ...
    fn create_buffer() -> std::io::BufWriter<Vec<u8>> { std::io::BufWriter::new(Vec::new()) }
