...
use api_config::{Config, DatabaseConfig, Environment, load_config, parse_env};
    ...
    match config {
        Ok(config) => match cli.command {
            ...
            Commands::Prepare => {
                ...
                let cargo = get_cargo_path().expect(
                    "Existence of CARGO env var is asserted by calling \
                     `ensure_sqlx_cli_installed`",
                );
                ...
                if !o.status.success() {
                    let error = anyhow!(String::from_utf8_lossy(&o.stdout).to_string())
                        .context(
                            "Error generating query metadata. Are you sure the database is \
                             running and all migrations are applied?",
                        );
                    return Err(error);
                }

                ui.success(
                    "Query data written to db/.sqlx directory; please check this into version \
                     control.",
                );
                Ok(())
            }
...
async fn ensure_sqlx_cli_installed(ui: &mut UI<'_>) -> Result<(), anyhow::Error> {
    ...
    async fn installed_sqlx_cli_version(cargo: &str) -> Result<Option<Version>, anyhow::Error> {
        ...
        fn error_parsing_version() -> anyhow::Error {
            anyhow!(
                "Error parsing sqlx-cli version. Please install the correct version manually \
                 using `cargo install sqlx-cli --version ^{SQLX_CLI_VERSION} --locked`"
            )
        }
    ...
    ui.info(&format!(
        "This command requires a version of sqlx-cli that is compatible with version \
         ...
         install the latest compatible version now? [Y/n]"
    ));
...
/// Find the root of the db package in the this project workspace.
