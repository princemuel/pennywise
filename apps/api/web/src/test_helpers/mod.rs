...
impl TestRequest {
    ...
    /// Sets the HTTP method for the request, e.g. GET or POST, see
    /// [`axum::http::Method`].
    #[allow(unused)]
    ...
    ///
    /// Header names must be passed as [`hyper::header::HeaderName`] while
    /// values can be passed as [`&str`]s.
    ///
    /// Example:
    /// ```
    /// let response = context
...
impl RouterExt for Router {
    #[allow(unused)]
    fn request(&self, uri: &str) -> TestRequest { TestRequest::new(self.clone(), uri) }
}
...
///
/// A `DbTestContext` is passed as an argument to tests marked with the
/// [`api_macros::db_test`] attribute macro. It is used to access the
/// application under test as well as the database (which is the same database
...
pub struct DbTestContext {
    ...
    pub app:     Router,
    /// A connection pool connected to the same database that the application
    /// that is being tested uses as well.
    pub db_pool: DbPool,
}

/// Sets up a test and returns a [`DbTestContext`] configured for the particular
/// test case.
///
/// This function initializes a new instance of the application under test using
/// the configuration for [`api_config::Environment::Test`]. The application is
/// configured to use the same database that is also made available to the test
/// itself via the test context. That database is a clone of the main test
/// database that is only used by the particular test case to ensure isolation
/// between test cases. It is automatically torn down after the test case
/// completes (see [`teardown`]).
///
/// This function is not invoked directly but used inside of the
/// [`api_macros::db_test`] attribute macro. The test context is automatically
/// passed to test cases marked with that macro as an argument.
#[allow(unused)]
