...
<section class="grid auto-cols-fr gap-8 sm:grid-cols-3">
	<hgroup class="flex flex-col gap-3 rounded-xl bg-grey-900 p-6 xl:p-8">
		<h4 class="text-sm text-white">Current Balance</h4>
		<p class="text-3xl font-bold text-white">${data.balance.current.toFixed(2)}</p>
	</hgroup>

	<hgroup class="md flex flex-col gap-3 rounded-xl bg-white p-6 xl:p-8">
		<h4 class="text-sm text-grey-500">Income</h4>
		<p class="text-3xl font-bold text-grey-900">${data.balance.income.toFixed(2)}</p>
	</hgroup>

	<hgroup class="flex flex-col gap-3 rounded-xl bg-white p-6 xl:p-8">
		<h4 class="text-sm text-grey-500">Expenses</h4>
		<p class="text-3xl font-bold text-grey-900">${data.balance.expenses.toFixed(2)}</p>
	</hgroup>
</section>
...
<section class="@container grid auto-rows-auto grid-cols-5 gap-8">
	...
	>
		...
		<div class="col-span-full grid gap-6 @2xl:grid-cols-5">
			<hgroup class="col-span-full rounded-xl bg-beige-100 p-6 @2xl:col-span-2">
			...
			<div class="col-span-full grid auto-cols-fr grid-cols-2 gap-4 @2xl:col-span-3">
				{#each data.pots.slice(0, 4) as pot (pot.id)}
					<div class="flex items-center gap-4" style="--theme:{pot.theme};">
						<div class="h-full w-1 rounded-full bg-(--theme)"></div>
