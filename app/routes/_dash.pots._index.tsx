...
import type { Route } from "./+types/_dash.pots._index";
...
export default function Page({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <>
      ...
      <section className="grid gap-8 @md:grid-cols-[repeat(auto-fit,minmax(24rem,1fr))]">
        {data.map((pot) => (
          <article
            key={pot.id}
            className="flex flex-col gap-8 rounded-xl bg-white p-6"
            style={{ "--color": pot.theme, anchorScope: "--anchor-pot" }}
          >
            <header className="flex items-center gap-4">
              <span className="rounded-full bg-(--color) p-2"></span>
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                <div
                  className="h-full w-(--width) rounded-full bg-(--color)"
